...
describe FoundationRailsHelper::FlashHelper do
  ...
  FoundationRailsHelper::FlashHelper::DEFAULT_KEY_MATCHING.each do |m, f|
    it "displays flash message with #{f} class for #{m} message" do
      allow(self).to receive(:flash).and_return(m.to_s => "Flash message")
      node = Capybara.string display_flash_messages
      expect(node)
        .to  have_css("div.flash.callout.#{f}", text: "Flash message")
